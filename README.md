...
这是一个同时包含**真实设备控制**、**测试脚本**、**路径规划仿真**和**历史实验代码**的仓库。

当前阶段请把它理解成两条线：

- **研发主线：`simulation/`** —— 当前主要工作面，负责算法验证、语义地图、协同重规划
- **真实系统线：`src/` + `tests/`** —— 真机控制与硬件诊断，不是当前仿真需求的主要改动对象

## 当前主线

- 当前主要研发主线在 `simulation/`：三张地图、语义修正、相切触发、重规划都在这里。
- `simulation/` 当前采用“**相切即触发、遇阻才重算**”机制，并且路径会在可行前提下尽量退化为直线段，避免网格抖动。
- 真实运行主线在 `src/`：无人机端、地面站、机器狗适配器、公用通信模块都在这里。
- 验证与诊断主线在 `tests/`：这里保留真实飞控和硬件测试脚本。
...
| `simulation/` | 路径规划仿真与实验输出 | **当前主线**，包含三图建模、事件触发、评估、可视化 |

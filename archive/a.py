...
def hold_with_monitor(seconds, **rc):
    """
    在给定 RC 输入下保持一段时间，同时每秒输出 RC override 状态
    """
    start = time.time()
    last_print = 0
...
# 监听遥控器通道变化
# =====================
def rc_listener(self, attr_name, value):
    ch1 = value.get('1', None)
    ch2 = value.get('2', None)
    ch3 = value.get('3', None)
    ch4 = value.get('4', None)
    print(f"[RC Monitor] CH1={ch1}, CH2={ch2}, CH3={ch3}, CH4={ch4}, Armed={vehicle.armed}")

vehicle.add_attribute_listener('channels', rc_listener)
...
# 持续监控直到遥控器上锁（disarm）
while vehicle.armed:
    ch1 = vehicle.channels.get('1', 0)
    ch2 = vehicle.channels.get('2', 0)
    ch3 = vehicle.channels.get('3', 0)
    ch4 = vehicle.channels.get('4', 0)
    print(f"[RC Monitor] CH1={ch1}, CH2={ch2}, CH3={ch3}, CH4={ch4}, Armed={vehicle.armed}")
    time.sleep(1)
